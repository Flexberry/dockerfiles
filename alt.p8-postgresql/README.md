...
Работа с контейнером возмоджна в двуз режимах:
- используя TCP порт postgres-сервера;
- используя клиентское приложение `psql` контейнера.

#### Работа с контейнером через TCP-порт
...
#### Работа с контейнером через клиентское приложение `psql` контейнера

##### Работа с приложением в 'ручном' режиме

Для запуска клиентского приложения `psql` контейнера наберите команду (где postgres - имя сервисв `docker-compose.yml`-файла):
```
docker compose exec postgres psql -U postgres
```
`psql` выдаст подсказку:
```
psql (9.6.12)
...
postgres=# 
```
Пароль при запуске в этом режиме не требуется.

Используйте команду `\?` для получения списка вспомонательных команд `psql`.
Основные команды:
- \l - вывести список баз данных;
- \c имя_базы - подключиться к указанной базе;; 
- \dt - вывести список таблиц базы
- \d имя_таблицы - вывести информацию о таблице

В клиенте `psql` Вы можете запускать любые допустимые SQL-операторы Postgres: создание пользователей, баз данных,
таблиц, индексов,корректировки и извлечения данных.

##### Формирование базы из DUMP'а

Если у Вас есть готовый текстовый DUMP баз данных, вы можете из сформировать запустив команду:
```
docker compose exec postgres psql -U postgres <имя_файла_дампа
```
